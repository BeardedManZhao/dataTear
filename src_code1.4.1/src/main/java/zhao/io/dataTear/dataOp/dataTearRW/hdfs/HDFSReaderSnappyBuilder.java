...
public class HDFSReaderSnappyBuilder implements Builder<HDFSReaderSnappy> {
     ...
     */
    @Priority("2")
    public HDFSReaderSnappyBuilder setIP_port(String IP, String port) {
     ...
     */
    @Priority("2")
    public HDFSReaderSnappyBuilder setKV(String key, String value) {
     ...
     */
    @Priority("3")
    public HDFSReaderSnappyBuilder setPathString(String pathString) {
     ...
     */
    @Priority("2")
    public HDFSReaderSnappyBuilder setPathObject(Path pathObject) {
     ...
     */
    @Priority("1")
    public HDFSReaderSnappyBuilder setFileSystem(FileSystem fileSystem) {
        this.fileSystem = fileSystem;
        return this;
    }

    @Override
    public HDFSReaderSnappy create() {
        try {
            if (fileSystem == null) fileSystem = FileSystem.get(configuration);
            if (pathObject == null) pathObject = new Path(pathString);
            return new HDFSReaderSnappy(fileSystem, pathObject, pathString);
        } catch (IOException e) {
            e.printStackTrace();
            return null;
        }
